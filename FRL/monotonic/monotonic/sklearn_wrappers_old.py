...
#     def fit(self, X, y, x_names=None):
#         """
#         can pass in names of each feature as x_names if desired
#         """
#         assert len(X) == len(y)
#         train_x_ns = monotonic_utils.nparray_to_x_data(X, x_names)
#         train_y_ns = monotonic_utils.nparray_to_y_data(y)
#         return monotonic_sklearn_predictor(self.classifier_constructor(train_x_ns, train_y_ns))
    ...
    def sample(self,n_start,n_sample,len_max=None,truncate=False):
        ...
        if truncate:
            ...
            for theta in theta_samples:
                while len(theta.rule_f_ls)>len_max:
                    theta.rule_f_ls.pop(len(theta.rule_f_ls)-1)
                theta.gamma_ls = theta.get_greedy_optimal_gamma_ls(self.x_ns, self.y_ns)
                theta_samples_new.append(copy.deepcopy(theta))
            return theta_samples_new
        else:
